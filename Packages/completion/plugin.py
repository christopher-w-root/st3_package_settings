...
def do_query(context, callback, driver, args, launch_daemon, daemon_command, debug=False):
    ...
    print("CROOT - completion - in do_query() - context: %s, callback: %s, driver: %s, launch_daemon: %d, daemon_command: %s, debug: %d" % (context, callback, driver, launch_daemon, daemon_command, debug))

    for i in range(2):
        # The reason for the loop here is to try and start the daemon if it wasn't running and the user settings allows this
        s = time.time()
        try:
            print ("CROOT - completion - in do_query() - driver.completeAt args: %s" % (args))
            response = driver.CompleteAt(args)
            break
        except jsonrpc.RPCFault as e:
            print ("CROOT - completion - in do_query() - RPCFault error!!")
            print(e.error_data)
            return
        except jsonrpc.RPCTransportError as e2:
            print ("CROOT - completion - in do_query() - RPCTransportError error!!")
...
def prepare_request(view, prefix, locations, settings):
    ...
    args = {
        ...
        "SessionOverrides": {
            ...
            "compiler_flags": view.settings().get("sublimeclang_options", ["-E"]),
...
class CompletionGotoDefinitionCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        print("CROOT - completion - in CompletionGotoDefinitionCommand.run() - prefix: %s, driver: %s, args: %s" % (prefix, driver, args))
...
class Ev(sublime_plugin.EventListener):
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        daemon_command = settings.get("daemon_command")
        debug = settings.get("debug", False)
        print("CROOT - completion - in Ev.on_query_completions() - launch_daemon: %d, daemon_command: %s" % (launch_daemon, daemon_command))
